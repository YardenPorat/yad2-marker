...
window.onload = async function () {
    ...
    function hideAllAds() {
        ...
        adsArray = Array.from(
            document.querySelectorAll(
                '.yad1_feed_item.premium, .yad1_row.top, .recommended_agencies, .inner_aside_container, .homeprices, #desktop-top-banners, .top_boxes_row.inactive, .y1_right_side.dynamic_cmp.accordion_wide_theme, .dynamic_cmp_wrapper.size_x1_3.gray_back, .feed-list-platinum, .dominant_realtor'
            )
        );
    ...
    function addXbuttonListeners() {
        ...
        for (let i = 0; i < closebtns.length; i++) {
            closebtns[i].addEventListener('click', function (event) {
                ...
                this.nextSibling.firstElementChild.style.backgroundColor =
                    'gray';
    ...
    function searchHideOnLoad() {
        ...
        savedData.filtered.forEach(postID => {
            ...
            const postElToHide = document.querySelector(
                `[item-id="${postID}"]`
            );
            if (postElToHide) {
